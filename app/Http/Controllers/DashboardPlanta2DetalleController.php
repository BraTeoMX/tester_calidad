...
class DashboardPlanta2DetalleController extends Controller
{
    /**
    ...
    public function dashboardPlanta2Detalle(Request $request)
    {
        $title = "";
        ...
        $porcentajesAQL = $semanas->map(function($semana) {
            ...
            return $this->calcularPorcentajePorSemana(AuditoriaAQL::class, $year, $week, 'Intimark2');
        });

        $porcentajesProceso = $semanas->map(function($semana) {
            list($year, $week) = explode('-', $semana);
            return $this->calcularPorcentajePorSemana(AseguramientoCalidad::class, $year, $week, 'Intimark2');
        });
        // Datos para las gráficas de clientes
        $dataGrafica = $this->obtenerDatosClientesPorRangoFechas($fechaInicio, $fechaFin, 'Intimark2');
        ...
        $dataGerentesAQLGeneral = $this->getDataGerentesProduccionAQL($fechaInicio, $fechaFin, 'Intimark2');
        $dataGerentesProcesoGeneral = $this->getDataGerentesProduccionProceso($fechaInicio, $fechaFin, 'Intimark2');
        $dataGerentesGeneral = $this->combineDataGerentes($dataGerentesAQLGeneral, $dataGerentesProcesoGeneral);

        // Datos para módulos
        $dataModuloAQLGeneral = $this->getDataModuloAQL($fechaInicio, $fechaFin, 'Intimark2');
        $dataModuloProcesoGeneral = $this->getDataModuloProceso($fechaInicio, $fechaFin, 'Intimark2');
        $dataModuloAQLPlanta1 = $this->getDataModuloAQL($fechaInicio, $fechaFin, 'Intimark2');
        $dataModuloAQLPlanta2 = $this->getDataModuloAQL($fechaInicio, $fechaFin, 'Intimark2');
        $dataModuloProcesoPlanta1 = $this->getDataModuloProceso($fechaInicio, $fechaFin, 'Intimark2');
    ...
    private function calcularTotales($dataClientes, $fechaInicio, $fechaFin)
    {
        $totalAuditadaAQL = array_sum(array_map(function ($data) use ($fechaInicio, $fechaFin) {
            return AuditoriaAQL::where('cliente', $data['cliente'])->where('planta', 'Intimark2')->whereBetween('created_at', [$fechaInicio, $fechaFin])->sum('cantidad_auditada');
        }, $dataClientes));

        $totalRechazadaAQL = array_sum(array_map(function ($data) use ($fechaInicio, $fechaFin) {
            return AuditoriaAQL::where('cliente', $data['cliente'])->where('planta', 'Intimark2')->whereBetween('created_at', [$fechaInicio, $fechaFin])->sum('cantidad_rechazada');
        }, $dataClientes));

        $totalAuditadaProceso = array_sum(array_map(function ($data) use ($fechaInicio, $fechaFin) {
            return AseguramientoCalidad::where('cliente', $data['cliente'])->where('planta', 'Intimark2')->whereBetween('created_at', [$fechaInicio, $fechaFin])->sum('cantidad_auditada');
        }, $dataClientes));

        $totalRechazadaProceso = array_sum(array_map(function ($data) use ($fechaInicio, $fechaFin) {
            return AseguramientoCalidad::where('cliente', $data['cliente'])->where('planta', 'Intimark2')->whereBetween('created_at', [$fechaInicio, $fechaFin])->sum('cantidad_rechazada');
        }, $dataClientes));
    ...
    private function obtenerDatosModulosPorRangoFechas($fechaInicio, $fechaFin)
    {
        ...
        foreach ($period as $week) {
            ...
            $queryAQL = AuditoriaAQL::whereNotNull('modulo')->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2');
            $queryProceso = AseguramientoCalidad::whereNotNull('modulo')->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2');
            ...
            foreach ($modulosDelDia as $modulo) {
                ...
                $sumaAuditadaAQL = AuditoriaAQL::where('modulo', $modulo)->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2')->sum('cantidad_auditada');
                $sumaRechazadaAQL = AuditoriaAQL::where('modulo', $modulo)->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2')->sum('cantidad_rechazada');

                $porcentajeErrorAQL = ($sumaAuditadaAQL != 0) ? ($sumaRechazadaAQL / $sumaAuditadaAQL) * 100 : 0;

                $sumaAuditadaProceso = AseguramientoCalidad::where('modulo', $modulo)->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2')->sum('cantidad_auditada');
                $sumaRechazadaProceso = AseguramientoCalidad::where('modulo', $modulo)->whereBetween('created_at', [$startOfWeek, $endOfWeek])->where('planta', 'Intimark2')->sum('cantidad_rechazada');
    ...
    public function detalleXModuloPlanta2(Request $request)
    {
        $title = "";
        ...
        return view('dashboar.detalleXModuloPlanta2', compact('title', 'clienteBusqueda', 'semanas', 'datosCombinados', 'promediosGenerales'));
    }
}
